...
namespace TaxInvoiceManagment.Application.Managers
{
    public class TaxService : ITaxService
    {
        ...
        public TaxService(ILogger<TaxService> logger, IUnitOfWork unitOfWork, IMapper mapper, IValidator<TaxDto> taxDtoValidator)
        {
            _logger = logger;
        ...
        public async Task<Result<TaxDto>> CreateTax(TaxDto taxDto)
        {
            ...
            {
                var taxableItem = await _unitOfWork.TaxableItems.GetByIdAsync(taxDto.TaxableItemId);
                if (taxableItem == null)
                {
