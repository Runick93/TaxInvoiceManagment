...
namespace TaxInvoiceManagment.Application.Managers
{
    public class InvoiceService : IInvoiceService
    {
        ...
        public InvoiceService(ILogger<InvoiceService> logger, IUnitOfWork unitOfWork, IMapper mapper, IValidator<InvoiceDto> invoiceValidator)
        {
            _logger = logger;
        ...
        public async Task<Result<InvoiceDto>> CreateInvoice(InvoiceDto invoiceDto)
        {
            ...
            var tax = await _unitOfWork.Taxes.GetByIdAsync(invoiceDto.TaxId);
            if (tax == null)
            {
                _logger.LogError($"No se encontro el impuesto o servicio asociado al ID '{invoiceDto.TaxId}'.");
